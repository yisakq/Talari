...
contract Payment {
    ...
    function sendPayment(address receiver, uint256 _amount, string memory _message) public payable {
        require(receiver != address(0), "Invalid address");
        uint256 toSend = _amount * 1e18;
        require(msg.value == toSend, "Incorrect amount sent");
    ...
    function addHistory(address sender, address receiver, uint256 _amount, string memory _message) private {
        ...
        if (names[receiver].hasName) {
            newSend.otherPartName = names[receiver].name;
        }

        history[sender].push(newSend);

        sendReceive memory newReceive;
        newReceive.action = "received";
        newReceive.amount = _amount;
        newReceive.message = _message;
        newReceive.otherPartyAddress = sender;
        if (names[sender].hasName) {
            newReceive.otherPartName = names[sender].name;
        }

        history[receiver].push(newReceive);
    }

    function deleteRequest(uint256 _request) public {
        require(_request < requests[msg.sender].length, "No such request");
        request[] storage myRequests = requests[msg.sender];
